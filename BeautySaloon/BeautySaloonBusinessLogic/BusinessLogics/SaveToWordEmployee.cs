...
{
    static class SaveToWordEmployee
    {/// <summary>
     /// Создание документа
     /// </summary>
     /// <param name="info"></param>
        public static void CreateDoc(WordInfoEmployee info)
        {
            using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(info.FileName, WordprocessingDocumentType.Document))
            {
                ...
                foreach (var cosmetic in list)
                {
                    docBody.AppendChild(CreateParagraph(new WordParagraph
                    {
                        Texts = new List<(string, WordTextProperties)> { ("Наименование: " + cosmetic.Key, new WordTextProperties { Size = "24" }) },
                        TextProperties = new WordTextProperties
                        {
                            Size = "24",
                ...
                docBody.AppendChild(CreateParagraph(new WordParagraph
                {
                    Texts = new List<(string, WordTextProperties)> { ("Общая сумма покупок: " + info.Purchases[0].TotalCost, new WordTextProperties { Size = "24" }) },
                    TextProperties = new WordTextProperties
                    {
                        Size = "24",
                        JustificationValues = JustificationValues.Both
